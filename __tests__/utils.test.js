...
describe("controller utils", () => {
    describe("checkExists", () => {
        test("Returns a promise resolved if value passed in is found within table", async () => {
            await expect(checkExists("users", "name", "sam")).resolves.not.toThrow();
            await expect(checkExists("articles", "title", "Z")).resolves.not.toThrow();
            await expect(checkExists("topics", "slug", "paper")).resolves.not.toThrow();
        });
        test("Returns a promise reject if table_name passed is invalid", async () => {
            await expect(checkExists("unicorns", "slug", "paper")).rejects.toEqual(expect.anything());
        });
        test("Returns a promise reject if column_name passed is invalid", async () => {
            await expect(checkExists("topics", "unicorn", "paper")).rejects.toEqual(expect.anything());
        });
        test("Returns a promise reject if value passed is invalid", async () => {
            await expect(checkExists("topics", "slug", "butterfly")).rejects.toEqual(expect.anything());
        });
        test("Returns a promise reject if multiple arguments are invalid", async () => {
            await expect(checkExists("unicorns", "unicorns", "unicorn")).rejects.toEqual(expect.anything());
        });
    });
});
